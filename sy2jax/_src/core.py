    ...
    *, 
    ...
    for clz in type(expr).__mro__:
        if clz in function_table:
            return function_table[clz](expr, symbol_table=symbol_table, function_table=function_table)
    raise NotImplementedError(f"Implementation missing for {type(expr)}")
...
def unary_op(op: UnaryOp) -> Translation:
    def fun(expr, **kwargs):
        if  len(expr.args) != 1:
            raise ValueError(f"Trying to call unary op {type(expr)} with arguments {expr.args}")
